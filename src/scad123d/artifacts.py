...
class ArtifactPass:
    ...
    def plan(self) -> tuple[list[Job], list[Job]]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    originals, duplicates = artifact_pass.plan()
    pending = sum(1 for j in originals + duplicates if artifact_pass._needs_stl(j))
    print(
        f"scad123d-artifacts: {len(originals) + len(duplicates)} inputs in "
        f"{sorted(artifact_pass.statuses)}, {len(duplicates)} duplicates, "
