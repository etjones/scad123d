...
def test_env_override_wins_when_it_exists(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        osc.shutil, "which", lambda name: pytest.fail("should not search PATH")
    )
...
def test_path_is_searched_before_platform_candidates(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        osc.shutil, "which", lambda name: str(on_path) if name == "openscad" else None
    )
    monkeypatch.setattr(osc.platform, "system", lambda: "Darwin")
    monkeypatch.setattr(
        Path, "exists", lambda self: pytest.fail("should not check candidate paths")
    )
...
def test_matching_version_does_not_skip(monkeypatch):
    ...
    # copy.
    monkeypatch.setattr(
        conftest, "openscad_version", lambda: "OpenSCAD version 2025.07.18"
    )
    conftest.require_fixture_openscad_version(
        {"_openscad_version": "OpenSCAD version 2025.07.18"}
    )
...
def test_mismatched_version_skips(monkeypatch):
    ...
    monkeypatch.setattr(
        conftest, "openscad_version", lambda: "OpenSCAD version 2021.01"
    )
